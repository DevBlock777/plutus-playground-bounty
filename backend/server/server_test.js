...
// ══════════════════════════════════════════════════════════
//  DOCKER ARCHITECTURE — Ephemeral build containers (Option A)
//
//  RUNNER_IMAGE         : image used to spawn per-build containers
//  WORKSPACE_CONTAINER  : long-lived container for user workspace I/O
//
//  Per-build lifecycle:
//    1. docker run --rm -d --name plutus-job-<jobId> RUNNER_IMAGE sleep infinity
//    2. docker cp Main.hs into the job container
//    3. docker exec plutus-job-<jobId> cabal run alw-exe
//    4. docker cp <jobId>output.plutus out
//    5. container auto-removed by --rm (or killed on timeout)
//
//  Result: builds from different users NEVER share a filesystem.
// ══════════════════════════════════════════════════════════
const RUNNER_IMAGE = process.env.PLUTUS_RUNNER_IMAGE || "plutus-nix-runner";
const WORKSPACE_CONTAINER =
    process.env.PLUTUS_WORKSPACE_CONTAINER || "plutus-workspace";
const BUILD_CPUS = process.env.BUILD_CPUS || "1.5";
const BUILD_MEMORY = process.env.BUILD_MEMORY || "3g";
...
/**
 * Spawn an ephemeral build container.
 * Returns the container name `plutus-job-<jobId>`.
 * --rm         : auto-deleted when stopped
 * --network none : no outbound network during compilation (security)
 * --cpus / --memory : prevent resource starvation
 */
async function spawnJobContainer(jobId) {
    const name = `plutus-job-${jobId}`;
    await execAsync(
...
/**
 * Kill and remove a job container (idempotent).
 * Safe to call even if the container was already removed by --rm.
 */
async function killJobContainer(name) {
    try {
        await execAsync(`docker rm -f "${name}" 2>/dev/null || true`);
    } catch (_) {}
}

// Toolchain versions — fetched once at startup from a throwaway container
let serverVersions = {
    ghc: "fetching…",
    cabal: "fetching…",
    nix: "fetching…",
...
function fetchVersions() {
    exec(
        `docker run --rm "${RUNNER_IMAGE}" bash -lc ` +
            `"source /root/.nix-profile/etc/profile.d/nix.sh && ghc --version && cabal --version && nix --version" 2>/dev/null`,
        (err, out) => {
            if (err || !out) return;
...
function ensureUserDir(userId, cb) {
    exec(
        `docker exec "${WORKSPACE_CONTAINER}" mkdir -p "${userWspace(userId)}"`,
        cb,
    );
}
...
app.get("/workspace/files", requireAuth, (req, res) => {
    ...
    ensureUserDir(req.session.user.id, () => {
        exec(
            `docker exec "${WORKSPACE_CONTAINER}" ls -p "${targetDir}"`,
            (err, stdout) => {
...
app.get("/workspace/file", requireAuth, (req, res) => {
    ...
    exec(
        `docker exec "${WORKSPACE_CONTAINER}" cat "${fullPath}"`,
        (err, stdout) => {
...
app.post("/workspace/create", requireAuth, (req, res) => {
    ...
    const cmd = `docker exec "${WORKSPACE_CONTAINER}" mkdir -p "${dirInContainer}" && docker cp "${tmpPath}" "${WORKSPACE_CONTAINER}":"${userWspace(req.session.user.id)}/${filePath}"`;
    exec(cmd, (err) => {
...
app.post("/workspace/save", requireAuth, (req, res) => {
    ...
    fs.writeFileSync(tmpPath, content);
    const cmd = `docker cp "${tmpPath}" "${WORKSPACE_CONTAINER}":"${userWspace(req.session.user.id)}/${filePath}"`;
    exec(cmd, (err) => {
        if (fs.existsSync(tmpPath)) fs.unlinkSync(tmpPath);
        if (err) return res.status(500).send(err.message);
        res.send("Saved");
...
app.post("/workspace/mkdir", requireAuth, (req, res) => {
    ...
    exec(
        `docker exec "${WORKSPACE_CONTAINER}" mkdir -p "${fullPath}"`,
        (err) => {
            if (err) return res.status(500).send(err.message);
...
app.delete("/workspace/delete", requireAuth, (req, res) => {
    ...
    const cmd = isDirectory
        ? `docker exec "${WORKSPACE_CONTAINER}" rm -rf "${fullPath}"`
        : `docker exec "${WORKSPACE_CONTAINER}" rm -f "${fullPath}"`;
    exec(cmd, (err) => {
        if (err) return res.status(500).send(err.message);
        res.send("Deleted");
    });
...
app.get("/workspace/search", requireAuth, async (req, res) => {
    ...
    const base = userWspace(req.session.user.id);
    const cmd = `docker exec "${WORKSPACE_CONTAINER}" find "${base}" -name "*.hs" -exec grep -l "${query}" {} \\;`;
    ...
    try {
        ...
        const promises = files.map(
            (file) =>
                new Promise((resolve) => {
                    exec(
                        `docker exec "${WORKSPACE_CONTAINER}" grep -n "${query}" "${file}"`,
                        (err, out) => {
                            if (!err && out) {
                                const lines = out
                                    .split("\n")
...
app.post("/compile", requireAuth, async (req, res) => {
    ...
    // Timeout watchdog — also kills the ephemeral job container
    function startTimeout(child, containerName) {
        jobTimeout = setTimeout(async () => {
            ...
            if (containerName) await killJobContainer(containerName);
            release();
            endSSE(res);
        }, JOB_TIMEOUT_MS);
    }
    ...
    // ────────────────────────────────
    //  MODE FICHIER  (ephemeral container per build)
    // ────────────────────────────────
    if (fileName) {
        ...
        // 1. Read source from the WORKSPACE container (always-on)
        exec(
            `docker exec "${WORKSPACE_CONTAINER}" test -f "${sourceFile}" && echo "FOUND" || echo "MISSING"`,
            (err, out) => {
                if (out.trim() === "MISSING") {
                    sendSSE(
                        res,
                ...
                exec(
                    `docker exec "${WORKSPACE_CONTAINER}" cat "${sourceFile}"`,
                    ...
                    (err2, sourceCode) => {
                        ...
                        (async () => {
                            // 2. Cache check (skips container spawn entirely on hit)
                            ...
                            // 3. Spawn EPHEMERAL build container (isolated per build)
                            let jobContainerName;
                            try {
                                sendSSE(
                                    res,
                                ...
                                jobContainerName =
                                    await spawnJobContainer(jobId);
                                sendSSE(
                                    res,
                                    `> Container ${jobContainerName} ready.\n`,
                                    "compilation",
                                );
                            } catch (spawnErr) {
                                fs.existsSync(tmpSrc) && fs.unlinkSync(tmpSrc);
                                sendSSE(
                                    res,
                                    `> ERROR: Could not spawn build container: ${spawnErr.message}\n`,
                                    "compilation",
                                );
                            ...
                            // 4. Copy augmented Main.hs into the ephemeral container
                            const lectureDir = `/app/code/wspace/lecture`;
                            const dockerCmd =
                                `docker cp "${tmpSrc}" "${jobContainerName}":${lectureDir}/Main.hs && ` +
                                `docker exec "${jobContainerName}" bash -lc "` +
                            ...
                            startTimeout(child, jobContainerName);
                            ...
                            req.on("close", async () => {
                                ...
                                child.kill();
                                release();
                                await killJobContainer(jobContainerName);
                            });
                        })();
                    },
                );
    ...
    // ────────────────────────────────
    //  MODE ÉDITEUR  (ephemeral container per build)
    // ────────────────────────────────
    if (!code) {
    ...
    // Cache check — avoids container spawn entirely on hit
    ...
    // Spawn EPHEMERAL build container (fully isolated, no shared filesystem)
    let jobContainerName;
    try {
        sendSSE(res, `> Spawning build container...\n`, "compilation");
        jobContainerName = await spawnJobContainer(jobId);
        sendSSE(res, `> Container ${jobContainerName} ready.\n`, "compilation");
    } catch (spawnErr) {
        try {
...
function handleClose(
    ...
    jobContainerName = null, // ephemeral container to read artefact from then destroy
) {
    if (exitCode === null || exitCode === 0) {
        sendSSE(res, "> Extracting CBOR...\n", "stdout");
        exec(
            `docker exec "${jobContainerName || WORKSPACE_CONTAINER}" cat /app/code/wspace/assets/${jobId}output.plutus`,
            async (err, stdout) => {
                if (err) {
                    sendSSE(
                        res,
                ...
                // Cleanup: destroy the ephemeral build container
                if (jobContainerName)
                    killJobContainer(jobContainerName).catch(() => {});
                endSSE(res);
            },
        );
    } else {
        ...
        // Cleanup: destroy the ephemeral build container even on failure
        if (jobContainerName)
            killJobContainer(jobContainerName).catch(() => {});
        endSSE(res);
    }
...
function handleCloseWithMain(
    ...
    sourceHash,
    displayFile,
    jobContainerName = null, // ephemeral container to read from then destroy
) {
    if (exitCode !== null && exitCode !== 0) {
        sendSSE(
            res,
    ...
    if (outputPaths.length === 0) {
        ...
        exec(
            `docker exec "${jobContainerName || WORKSPACE_CONTAINER}" find ${WSPACE}/assets -name "*.plutus" -o -name "*.json" 2>/dev/null`,
            async (err, out) => {
                const found = (out || "")
                    .split("\n")
                    .map((s) => s.trim())
...
async function serveOutputFiles(
    ...
    for (const relPath of outputPaths) {
        ...
        await new Promise((resolve) => {
            exec(
                `docker exec "${jobContainerName || WORKSPACE_CONTAINER}" cat "${fullPath}"`,
    ...
    // Cleanup: destroy the ephemeral build container
    if (jobContainerName) killJobContainer(jobContainerName).catch(() => {});
    endSSE(res);
}
